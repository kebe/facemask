...
class Net(nn.Module):
    ...
    def train(dataset):
        ...
        torch.save(train_images_tensor, 'tensor_datav2_840/train_images_tensor.pt')
        torch.save(train_labels_tensor, 'tensor_datav2_840/train_labels_tensor.pt')
        torch.save(val_images_tensor, 'tensor_datav2_840/val_images_tensor.pt')
        torch.save(val_labels_tensor, 'tensor_datav2_840/val_labels_tensor.pt')
        torch.save(test_images_tensor, 'tensor_datav2_840/test_images_tensor.pt')
        torch.save(test_labels_tensor, 'tensor_datav2_840/test_labels_tensor.pt')

        # train_images_tensor = torch.load('tensor_datav2/train_images_tensor.pt')
        # train_labels_tensor = torch.load('tensor_datav2/train_labels_tensor.pt')
        # val_images_tensor = torch.load('tensor_datav2/val_images_tensor.pt')
        # val_labels_tensor = torch.load('tensor_datav2/val_labels_tensor.pt')
        # test_images_tensor = torch.load('tensor_datav2/test_images_tensor.pt')
        # test_labels_tensor = torch.load('tensor_datav2/test_labels_tensor.pt')
        #import pdb; pdb.set_trace()
        net = Net()
    ...
    for i,j in enumerate(no_of_images):
        ...
        if type(doc["annotation"]["object"])!=list:
            ...
            if label == 1 and without_mask_count < total_size :
                ...
                image_tensor.append(my_transform(image))
                label_tensor.append(torch.tensor(label))
            if label == 2 and without_mask_count < total_size:
                without_mask_count +=1
                print('without_mask_count:', without_mask_count)
                image=transforms.functional.crop(Image.open(path_images+j).convert("RGB"), b,a,d-b,c-a)
                image_tensor.append(my_transform(image))
                label_tensor.append(torch.tensor(1))
        else:
            temp=doc["annotation"]["object"]
...
if __name__ == "__main__":
    ...
    options={"with_mask":0,"without_mask":1,"mask_weared_incorrect":1} 
