...
class Net(nn.Module):
    ...
    def test(input_tensor, output_tensor, PATH):
        ...
        #import pdb; pdb.set_trace()
        image = input_tensor[44];npimg = image.numpy();plt.imshow(np.transpose(npimg, (1, 2, 0)));plt.show()
        image = input_tensor[45];npimg = image.numpy();plt.imshow(np.transpose(npimg, (1, 2, 0)));plt.show()
        image = input_tensor[46];npimg = image.numpy();plt.imshow(np.transpose(npimg, (1, 2, 0)));plt.show()
...
if __name__ == "__main__":
    ...
    PATH = "outputv2_840/epoch_18_trained.pth"
  
    test_input_tensor = torch.load('tensor_datav2_840/test_images_tensor.pt')
    test_output_tensor = torch.load('tensor_datav2_840/test_labels_tensor.pt')
